...
contract ShoeFyStaking is Ownable {
    ...
    function stakeIn(uint256 _amount) public {
		...
		// transfer fee 
		shoeFy.transferFrom(address(this), burnAddress, _amount.mul(burnFee).div(100));
		shoeFy.transferFrom(address(this), daoTreasury, _amount.mul(daoFee).div(100));
		shoeFy.transferFrom(address(this), liquidityLocker, _amount.mul(liquidityFee).div(100));
