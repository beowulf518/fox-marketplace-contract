...
contract ShoeFyNFTStaking is Ownable {
    ...
    function stakeIn(uint256 _tokenId, uint256 _price) public {
		...
		shoeFy.transferFrom(msg.sender, address(this), _price.mul(burnFee + rewardFee + liquidityFee + daoFee).div(100));
		shoeFy.transferFrom(address(this), burnAddress, _price.mul(burnFee).div(100));
		shoeFy.transferFrom(address(this), daoTreasury, _price.mul(daoFee).div(100));
		shoeFy.transferFrom(address(this), liquidityLocker, _price.mul(liquidityFee).div(100));
...
// 		require(allowed[address(this)][msg.sender] >= amount, "You do not have enought... try a lower amount !");
		require(tokenId >= 0, "Hmmm, stop thinking negative... and USE A POSITIVE AMOUNT");
		allowed[address(this)][msg.sender] -= 1;
		userApr[msg.sender] = apr;
